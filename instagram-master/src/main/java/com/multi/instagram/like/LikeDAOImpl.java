...
import com.multi.instagram.board.BoardUploadRowMapper;
...
public class LikeDAOImpl implements LikeDAO {
	...
	public List<LikeDTO> read(String likeId, String boardId) {
		...
		return template.query("select count(*) from like_table where board_id=?", new Object[] { boardId },
				new LikeRowMapper());
	}
	...
	public List<BoardDTO> like_List(int userId) {
		return template.query(
				"select * from board_Table where board_id in (select board_id from like_Table where user_id = ?);",
				new Object[] { userId }, new BoardRowMapper());
	}

	@Override
	public int count(String likeId) {
		int cnt = template.queryForObject("select count(*) from like_table where user_id=?", Integer.class);
	...
	public List<BoardFileDTO> select_like_board(int userId) {
		String sql = "SELECT c.* from content_table c JOIN like_table l ON c.board_id = l.board_id WHERE l.user_id = ?";
